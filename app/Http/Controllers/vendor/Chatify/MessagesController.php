...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if(!empty($fetch->avatar))
        {
            ...
            $avatar = \App\Models\Utility::get_file(config('chatify.user_avatar.folder') . $fetch->avatar);
        }
        else
        {
    ...
    public function getContacts(Request $request)
    {
        ...
        if($objUser->type == 'Owner')
        {
            //$members = User::where('type', '!=', 'client')->where('created_by', '=', $objUser->creatorId())->get();
             $members = User::where('created_by', '=', $objUser->ownerId())->where('type', '!=', 'Client')->get();
        }
        else
        {
            // $members = User::where('type', '!=', 'client')->where('created_by', '=', $objUser->creatorId())->where('id', '!=', $objUser->id)
            // ->orWhere('id', '=', $objUser->creatorId())->get();
            $members = User::where('created_by', '=', $objUser->ownerId())->where('id', '!=', $objUser->id)->where('type', '!=', 'Client')->orWhere('id', '=', $objUser->ownerId())->get();
        }
        ...
        return Response::json(
            [
                'contacts' => $users->count() > 0 ? $contacts : '<br><p class="message-hint"><span>' . __('Your user list is empty') . '</span></p>',
    ...
    public function getFavorites(Request $request)
    {
        ...
        return Response::json([
            ...
            'favorites' => $favorites->count() > 0
                ? $favoritesList
                : '<p class="message-hint"><span>' . __('Your favourite list is empty') . '</span></p>',
        ], 200);
    }
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('created_by' , \Auth::user()->id)->where('first_name', 'LIKE', "%{$input}%");
    ...
    public function getMessagePopup()
    {
        ...
        $messages = Message::whereIn(
            ...
            $query->select(\DB::raw('MAX(id)'))->from('ch_messages')->where('to_id', '=', $user->id)->where('seen', '=', 0);
        }
        )->orderBy('id', 'desc')->get();
    ...
    {
        $user = Auth::user();
        Message::where('to_id', '=', $user->id)->update(['seen' => 1]);
