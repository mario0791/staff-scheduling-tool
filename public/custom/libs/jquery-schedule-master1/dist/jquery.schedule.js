...
;(function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    add: function (parent, position, height, options) {
      ...
      var periodTimeinput = '<div class="schedule_avaabilty_input"><input type="text" class="jqs-period-time jqs-period-time-text w-100" value="'+ this.periodInit(position, position + height) +'"></div>';
    ...
    openOptions: function (event, period) {
      ...
      if (this.settings.periodColors && this.settings.periodColors.length > 0) {
        ...
        var avebiltty = ["Avebiltty", "Unavebiltty"];
        $.each(this.settings.periodColors, function (index, color) {
      ...
      $('.jqs-options-color', this.element).click(function () {          
        ...
        }).attr('data-avebiltty',avebiltty).find('.schedule_avaabilty_input .jqs-period-avebility-text').attr('value',avebiltty);
      });
