  ...
  $.extend(Plugin.prototype, {
    ...
    create: function () {
      ...
      for (var i = 0; i < this.settings.days; i++) {
        $('<td><div class="jqs-day" data_days="'+i+'"></div></td>').
      ...
      if (this.settings.mode === 'edit') {
        ...
        dayDuplicate = '<div class="jqs-day-duplicate d-none" title="' + this.settings.periodDuplicateButton +
          '"></div>';
      }
    ...
    add: function (parent, position, height, options) {
      ...
      var day_no = $(parent).attr('data_days');
      ...
      if (this.settings.mode === 'edit') {
        ...
        periodDuplicate = '<div class="jqs-period-duplicate d-none" title="' + this.settings.periodDuplicateButton +
          '"></div>';
      }

      
      var availability = 'unavailability';      
      if(options.backgroundColor == 'rgba(0, 200, 0, 0.5)') { 
        availability = 'availability';
      }
      var periodTitle = '<div class="jqs-period-title">' + options.title + '</div>';
      var periodTime = '<div class="jqs-period-time text-dark" data-time="' + this.periodInit(position, position + height) + '">' + this.periodInit(position, position + height) + '</div>';      
      //var periodTimeinput = '<div class="schedule_avaabilty_input" data-counter="'+this.uniqId1()+'" ><input type="text" class="jqs-period-time jqs-period-time-text w-100" name = "timetable['+day_no+']['+this.uniqId1()+'][time]" value="'+this.periodInit(position, position + height)+'"><input type="text" class="jqs-period-time jqs-period-days-text w-100"  value="'+day_no+'"><input type="text" class="jqs-period-time jqs-period-availability-text w-100" name="timetable['+day_no+']['+this.uniqId1()+'][availability]" value="'+availability+'"></div>';     
      var periodTimeinput = '<div class="schedule_avaabilty_input" data-counter="'+this.uniqId1()+'" ><input type="hidden" class="jqs-period-time jqs-period-time-text w-100" name = "timetable['+day_no+']['+this.uniqId1()+'][time]" value="'+this.periodInit(position, position + height)+'"><input type="hidden" class="jqs-period-time jqs-period-days-text w-100"  value="'+day_no+'"><input type="hidden" class="jqs-period-time jqs-period-availability-text w-100" name="timetable['+day_no+']['+this.uniqId1()+'][availability]" value="'+availability+'"></div>';     

      
      var period = $('<div class="jqs-period">' +
      '<div class="jqs-period-container">' + periodTime + periodTimeinput + periodTitle + periodRemove + periodDuplicate + '</div>' +
      '</div>').css({
        'top': position * this.periodPosition,
        'height': height * this.periodPosition
      ...
      if (this.settings.mode === 'edit') {
        ...
        period.draggable({
          ...
          drag: function (event, ui) {            
            ...
            $('.jqs-period-time-text', ui.helper).attr('value',$this.periodDrag(ui));
            $this.closeOptions();
            //console.log($this.periodDrag(ui));
          },
        ...
        if (this.settings.periodOptions) {
          period.click(function (event) {
            if ( !$(event.target).hasClass('jqs-period-remove') || !$(event.target).hasClass('jqs-period-duplicate') )
            {
    ...
    openOptions: function (event, period) {
      ...
      var titleInput = '<div class="jqs-options-title-container d-none">' +
      ...
      if (this.settings.periodColors && this.settings.periodColors.length > 0) {
        ...
        var availability = ["Availability", "Unavailability"];
        $.each(this.settings.periodColors, function (index, color) {
          colorInput += '<button type="button" class="jqs-options-color mb-0 mt-3" ' +
            'style="background-color: ' + color[0] + '; border-color: ' + color[1] + '; color:' + color[2] + '">' +
            availability[index] + '</button>';
        });
        colorInput += '</div>';
      }

      // button
      var remove = '<div class="jqs-options-remove d-none">' + this.settings.periodRemoveButton + '</div>';
      var duplicate = '<div class="jqs-options-duplicate d-none">' + this.settings.periodDuplicateButton + '</div>';
      ...
      $('.jqs-options-color', this.element).click(function () {          
          ...
          if($(this).css('background-color') == 'rgba(0, 200, 0, 0.5)') { 
              availability = 'availability';
          }
        ...
        }).attr('data-availability',availability).find('.schedule_avaabilty_input .jqs-period-availability-text').attr('value',availability);
      });
    ...
    isValid: function (current) {
      ...
      //console.log(position * $this.periodPosition);
      // console.log(current.position());
      // console.log('current',current);
      // console.log('top',currentStart,'height',currentEnd);
      // console.log('*******************');
