    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $('.info-avatar').attr('src', data.user_avatar);
                ...
                messageInput.focus();
                var name=data.fetch.first_name.concat(" ", data.fetch.last_name);

                // update info in view
                $('.messenger-infoView .info-name').html(name);
                $('.m-header-messaging .user-name').html(name);
                // Star status
                data.favorite > 0
